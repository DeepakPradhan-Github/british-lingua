...
class Home extends CI_Controller
{
    ...
    public function shopping_cart2($gift_status = "")
    {
        ...
        $page_data['page_name'] = "weninar_cart";
        $page_data['page_title'] = site_phrase('webinar_cart');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function courses()
{
    ...
    if ($search_string == "" && $selected_category_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_language == 'all' && $selected_rating == 'all' && $selected_sorting == 'newest') {
        ...
        $this->db->where('type', 0); // Added condition for type = 0
    ...
    public function handle_buy_now($course_id = "")
    {
        ...
        if ($this->session->userdata('user_login')) {
            ...
            echo json_encode(['redirectTo' => site_url('sign_up')]);
        }
    }

    public function handle_buy_now2($course_id = "")
    {
        if ($this->session->userdata('user_login')) {
            $this->payment_model->configure_course_payment();
           //$this->load->view('frontend/default-new/webinar_cart.php'); // Replace 'webinar_cart' with your actual view name
            
            $page_data['page_name'] = 'webinar_cart';
        $page_data['page_title'] = site_phrase('webinar_cart');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);

        } else {
            set_url_history(site_url('home/handle_buy_now2'));
            $page_data['page_name'] = 'sign_up';
        $page_data['page_title'] = site_phrase('sign_up');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data); // Replace 'sign_up' with your actual view name
        }
    }
    
    public function handle_buy_now3($course_id = "")
    {
        if ($this->session->userdata('user_login')) {
            $this->payment_model->configure_course_payment();
           //$this->load->view('frontend/default-new/webinar_cart.php'); // Replace 'webinar_cart' with your actual view name
            ...
            $page_data['page_name'] = 'paid_demo';
        $page_data['page_title'] = site_phrase('paid_demo');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);

        } else {
            set_url_history(site_url('home/handle_buy_now3'));
            $page_data['page_name'] = 'sign_up';
        $page_data['page_title'] = site_phrase('sign_up');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data); // Replace 'sign_up' with your actual view name
        }
    ...
    {
        $page_data['page_name'] = 'managing_director';
        $page_data['page_title'] = site_phrase('managing_director');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function disclaimer()
    {
        $page_data['page_name'] = 'disclaimer';
        $page_data['page_title'] = site_phrase('disclaimer');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
     ...
     public function webinar()
    {
        ...
        $page_data['page_title'] = site_phrase('webinar');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function webinar2()
{
    // Load the 'get_top_courses' model
    $this->load->model('get_top_courses');

    // Retrieve top courses data
    $top_courses_data = $this->get_top_courses->result_array();

    // Set up data for the view
    $page_data['page_name'] = 'webinar';
    $page_data['page_title'] = site_phrase('webinar');
    $page_data['top_courses'] = $top_courses_data; // Pass top courses data to the view
    ...
    function course_payment2()
    {
        if (count($this->session->userdata('cart_items')) == 0) {
            $this->session->set_flashdata('error_message', site_phrase('there_are_no_courses_on_your_cart'));
            redirect(site_url('home/shopping_cart2'), 'refresh');
        }

        if (!$this->session->userdata('user_login')) {
            set_url_history(site_url('home/shopping_cart2'));
            redirect(site_url('login'), 'refresh');
        }
