...
// }
   ...
   // Category Sub Category Image Delete
    ...
    public function add_course($param1 = "")
    {
        ...
        $data['type'] = $this->input->post('web');
        $data['title'] = html_escape($this->input->post('title'));
        $data['Teacher'] = $this->input->post('Teacher');
        $data['Days'] = $this->input->post('Days');
        $data['Hourss'] = $this->input->post('Hourss');
        $data['class_url'] = $this->input->post('class_url');
    ...
    public function get_top_courses2()
{
    $scorm_status = addon_status('scorm_course');
    $h5p_status = addon_status('h5p');
    ...
    $this->db->where('is_top_course', 1);
    $this->db->where('status', 'active');
    $this->db->where('type', 1); // Add this line to filter by type value 1

    return $this->db->get('course');
}

public function get_top_courses3()
{
    $this->db->where('type', 1); // Filter by type value 1
    return $this->db->get('course');
}
    ...
    public function update_frontend_settings()
    {
        ...
        $data['value'] = $this->input->post('managing_director', false);
        $this->db->where('key', 'managing_director');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('disclaimer', false);
        $this->db->where('key', 'disclaimer');
        $this->db->update('frontend_settings', $data);
        ...
        $this->db->update('frontend_settings', $data);
         ...
         $data['value'] = $this->input->post('webinar', false);
        $this->db->where('key', 'webinar');
        $this->db->update('frontend_settings', $data);
    }
